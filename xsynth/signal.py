...
class SignalGenerator:
    def __init__(self, t, unit='bunches', dt=None, oscillator=None, relative_scan = False, **kwargs):
        ...
        if self.unit not in ['bunches', 'time']:
    ...
    def signal(self):
        ...
        return self.oscillator.generate(self.t, **self.signal_params)
...
class DACSignalGenerator(SignalGenerator):
    ...
    -----------
    kicker : xsynth
        An instance of the xsynth class that controls the DAC device.Provides time interval and signal duration.
    ...
    def __init__(self, kicker_device, beamline = None, oscillator = None, **kwargs):
        ...
        Parameters:
        ----------
        kicker_device (xsynth): 
            Instance of a xsynth class to write the generated signals. The kicker provides time interval and signal duration.
...
class ADAPTSignalGenerator(SignalGenerator):
    """
    ADAPTSignalGenerator

    A subclass of SignalGenerator for generating and writing signals to the adaptation server. This class specifically integrates with beamlines and kickers to generate 
    signals suitable for controlling devices in an accelerator or similar environment.

    Attributes:
    -----------
    kicker : xsynth
        An instance of the device class that controls the DAC device.Provides time interval and signal duration.
    beamline : str or None
        The identifier of the beamline to target for signal generation. Allowed values are: [None, "D", "1", "2", "3", "13", "4"].
    ...
    def __init__(self, adaptation_server, oscillator = None, **kwargs):
        ...
        assert self.beam_region in beam_regions, f"Specified Beamline does not exist. beamline should be in {beam_regions}"
    ...
    #     #signal_values = float_to_16bit_int(self.oscillator.generate(self.t, **self.signal_params))
    #     print(signal_values)
    #     print(self.server.pulseId)
    #     return xr.DataArray(signal_values, dims=['pulseId'], coords={'pulseId': self.server.pulseId}, attrs={'unit': 'Bunch Index'})
